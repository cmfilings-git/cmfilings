...
async function loadClients() {
    ...
    tbody.innerHTML = `<tr><td colspan="5" class="p-8 text-center text-slate-500 font-medium">Loading Client Data...</td></tr>`;
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            tr.className = "client-main-row hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors";
            tr.dataset.id = data.ClientID;
            
            // Build the Call Button (Updated to Brand Color)
            const callBtn = data.MobileNo 
                ? `<a href="tel:${data.MobileNo}" class="p-1.5 bg-brand-50 text-brand-600 rounded-lg hover:bg-brand-100 dark:bg-brand-900/20 dark:hover:bg-brand-900/40 transition" title="Call Client"><i data-lucide="phone" class="w-4 h-4"></i></a>` 
                : '';

            tr.innerHTML = `
                <td class="p-4 flex items-center gap-2 border-slate-100 dark:border-slate-800">
                    <button onclick="toggleExpand('${data.ClientID}')" class="text-slate-400 hover:text-slate-800 dark:hover:text-white transition p-1.5 bg-slate-50 dark:bg-slate-800 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700" title="View Details">
                        <i data-lucide="chevron-down" id="icon-${data.ClientID}" class="w-4 h-4 transition-transform duration-200"></i>
                    </button>
                    <button onclick="editClient('${data.ClientID}')" class="text-brand-600 hover:text-brand-800 dark:hover:text-brand-400 transition p-1.5 bg-brand-50 dark:bg-brand-900/20 rounded-md hover:bg-brand-100 dark:hover:bg-brand-900/40" title="Edit Client">
                        <i data-lucide="pencil" class="w-4 h-4"></i>
                    </button>
                </td>
                <td class="p-4 font-mono font-bold text-slate-900 dark:text-white">${data.ClientID}</td>
                <td class="p-4 font-semibold text-slate-800 dark:text-slate-200">${data.ClientName || '-'}</td>
                <td class="p-4 font-mono text-xs text-slate-500 dark:text-slate-400">${data.GSTNo || '-'}</td>
                <td class="p-4 flex items-center gap-3">
                    <span class="font-medium text-slate-700 dark:text-slate-300">${data.MobileNo || '-'}</span>
                    ${callBtn}
                </td>
            `;
            ...
            expandTr.className = "hidden bg-slate-50/50 dark:bg-slate-800/20 border-b border-slate-100 dark:border-slate-800";
            
            expandTr.innerHTML = `
                <td colspan="5" class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-sm">
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Legal Name</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.LegalName || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Email</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.Email || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Branch</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.Branch || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Trade Name</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.LegalTradeName || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Constitution</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.ConstitutionOfBusiness || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">PAN No</span><span class="font-mono text-slate-800 dark:text-slate-200">${data.PanNo || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Aadhaar No</span><span class="font-mono text-slate-800 dark:text-slate-200">${data.AdharNo || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Father's Name</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.FathersName || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">DOB / DOC</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.DOB || '-'}</span></div>
                        <div class="sm:col-span-2"><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Address</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.Address || '-'}</span></div>
                        <div class="sm:col-span-2"><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Notes</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.Notes || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Reference</span><span class="font-medium text-slate-800 dark:text-slate-200">${data.Reference || '-'}</span></div>
                        <div><span class="text-[10px] text-slate-400 uppercase font-bold block mb-1 tracking-wider">Folder Link</span>${data.FolderLink ? `<a href="${data.FolderLink}" target="_blank" class="text-brand-600 underline font-medium hover:text-brand-800">Open Folder</a>` : '-'}</div>
                    </div>
                </td>
            `;
        ...
        tbody.innerHTML = `<tr><td colspan="5" class="p-8 text-center text-brand-600 font-bold">Error loading data. Check console.</td></tr>`;
    }
}
