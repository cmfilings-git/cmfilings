...
document.addEventListener("DOMContentLoaded", () => {
    ...
    .then(([navHtml, formHtml]) => {
        ...
        // Secondary initialization calls dependent on DOM structural configurations
        initializeApplicationEngine();
    })
    .catch(error => {
        console.error("Critical Runtime Framework Error loading isolated assets structural fragments:", error);
    });
});

// ==========================================
// CENTRAL APPLICATION FRAMEWORK INITIALIZATION
// ==========================================
...
// Dark/Light Theme Switching Engine Logic 
...
// Navigation Tab Routing Systems Configuration
 Tariffs
...
// ==========================================
// CHART INTERACTION ENGINE LOGIC (CHART.JS)
// ==========================================
...
function synchronizeSheetDatabase() {
    ...
    Papa.parse(csvURL, {
        ...
        complete: function(results) {
            ...
            countDisplay.innerText = clientsDatabase.length;
            ...
            tbody.innerHTML = ""; // Wipe table buffer clean

            clientsDatabase.forEach((row, index) => {
                ...
                tr.innerHTML = `
                    ...
                    <td style="text-align: center;">
                        <button class="btn-row-action" style="background:rgba(37,99,235,0.1); color:var(--accent);" onclick="openViewModal(${index})"><i class="fa-solid fa-expand"></i> View</button>
                        <button class="btn-row-action" style="background:rgba(16,185,129,0.1); color:#10b981;" onclick="openEditModeModal(${index})"><i class="fa-solid fa-pen-to-square"></i> Edit</button>
                        <button class="btn-row-action" style="background:rgba(239,68,68,0.1); color:var(--danger);" onclick="executeRowDeletion(${index})"><i class="fa-solid fa-trash-can"></i> Delete</button>
                    </td>
                `;
...
// Central Dynamic Modal Display Handlers
...
// API Payload Transmission Framework Logic
function commitFormTransaction() {
    ...
    document.getElementById('tableStatus').innerText = "Transmitting records update execution parameters onto Google Cloud Networks...";
    closeFormModal();

    fetch(APPS_SCRIPT_WEBAPP_URL, {
        method: "POST",
        mode: "no-cors", // Bypasses browser client-side sandbox restrictions safely for automated App Script macros
    ...
    .then(() => {
        ...
        }, 2500); // Deferred reconciliation lag allowance 
    })
    .catch(err => alert("Transmission structural layer error logic traced: " + err));
}
...
function executeRowDeletion(index) {
    ...
    document.getElementById('tableStatus').style.display = 'block';
    document.getElementById('tableStatus').innerText = "Wiping selected file allocation table row references...";
    ...
    .catch(err => alert("Wipe API processing layer configuration error: " + err));
}
