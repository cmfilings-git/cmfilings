// Initialize Lucide Icons & Reference Data
let validStates = new Set();
let validHSN = new Set();
...
// Fetch HSN and State reference JSON files
async function loadValidationData() {
    try {
        let stateRes = await fetch('state.json');
        let stateData = await stateRes.json();
        if (stateData.state) {
            stateData.state.forEach(s => validStates.add(s.cd));
        }

        let hsnRes = await fetch('hsn.json');
        let hsnData = await hsnRes.json();
        if (hsnData.data) {
            hsnData.data.forEach(h => validHSN.add(h.c));
        }
        console.log("Validation data loaded successfully.");
    } catch (e) {
        console.warn("Could not load state.json or hsn.json. Validation will be skipped.", e);
    }
}
...
let errorLog = []; // Holds validation errors
...
function normalizeRows(rows) {
    return rows.map((row, index) => {
        let normalized = { __rowNum__: row.__rowNum__ || (index + 4) }; 
        for (let k in row) {
            if(k !== '__rowNum__') normalized[k.toLowerCase().trim()] = row[k];
        }
        return normalized;
...
function validatePOS(pos, section, rowNum) {
    if(pos && validStates.size > 0 && !validStates.has(pos)) {
        errorLog.push(`[${section.toUpperCase()}] Row ${rowNum}: Invalid POS code '${pos}'`);
    }
}
function validateHSN(hsn, section, rowNum) {
    if(hsn && validHSN.size > 0 && !validHSN.has(hsn)) {
        errorLog.push(`[${section.toUpperCase()}] Row ${rowNum}: Invalid HSN code '${hsn}'`);
    }
}
    ...
    try {
        if (section === 'b2cs') {
            ...
            rows.forEach(row => {
                ...
                validatePOS(pos, section, row.__rowNum__);
            ...
            rows.forEach(row => {
                ...
                let taxAmt = Number((txval * rt / 100).toFixed(2));
                let itm_det = { txval, rt, csamt: csamt };
                if(pos === userStateCode) { 
                    itm_det.camt = Number((taxAmt/2).toFixed(2)); 
                    itm_det.samt = Number((taxAmt/2).toFixed(2)); 
                    itm_det.iamt = 0;
            ...
            rows.forEach(row => {
                ...
                if(!hsn_sc) return;
                validateHSN(hsn_sc, section, row.__rowNum__);
...
function calculateAggregates() {
    let agg = {
        b2b: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        b2cl: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        b2cs: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        cdnr: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        cdnur: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        exp: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        at: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 }, 
        atadj: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        exemp: { count: 0, txval: 0 },
        hsn_b2b: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        hsn_b2c: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        eco: { count: 0, txval: 0, iamt: 0, camt: 0, samt: 0, csamt: 0 },
        docs: { count: 0 },
    ...
    gstData.b2b.forEach(ctin => {
        ...
        ctin.inv.forEach(inv => inv.itms.forEach(itm => {
            ...
            agg.b2b.csamt += itm.itm_det.csamt || 0;
        }));
    });
    ...
    gstData.b2cl.forEach(pos => {
        ...
        pos.inv.forEach(inv => inv.itms.forEach(itm => {
            ...
            agg.b2cl.csamt += itm.itm_det.csamt || 0;
        }));
    });
    ...
    gstData.cdnr.forEach(ctin => {
        ...
        ctin.nt.forEach(nt => nt.itms.forEach(itm => {
            ...
            agg.cdnr.csamt += itm.itm_det.csamt || 0;
        }));
    });
    ...
    gstData.cdnur.forEach(typ => {
        ...
        typ.nt.forEach(nt => nt.itms.forEach(itm => {
            ...
            agg.cdnur.csamt += itm.itm_det.csamt || 0;
        }));
    });
    ...
    gstData.exp.forEach(et => {
        ...
        et.inv.forEach(inv => inv.itms.forEach(itm => {
            ...
            agg.exp.csamt += itm.csamt || 0;
        }));
    });

    gstData.at.forEach(rec => {
        agg.at.count += rec.itms.length;
        rec.itms.forEach(itm => {
            agg.at.txval += itm.ad_amt || 0;
            agg.at.iamt += itm.iamt || 0;
            agg.at.camt += itm.camt || 0;
            agg.at.samt += itm.samt || 0;
            agg.at.csamt += itm.csamt || 0;
        });
    });

    gstData.atadj.forEach(rec => {
        agg.atadj.count += rec.itms.length;
        rec.itms.forEach(itm => {
            agg.atadj.txval += itm.ad_adj_amt || 0;
            agg.atadj.iamt += itm.iamt || 0;
            agg.atadj.camt += itm.camt || 0;
            agg.atadj.samt += itm.samt || 0;
            agg.atadj.csamt += itm.csamt || 0;
        });
    });
...
function downloadExcelSummary() {
    const gstinInput = document.getElementById('gstin').value.trim().toUpperCase() || 'N/A';
    const fpInput = document.getElementById('fp').value.trim() || 'N/A';
    const clientName = document.getElementById('clientName').value.trim() || 'N/A';
    const clientId = document.getElementById('clientId').value.trim() || 'N/A';
    
    const agg = calculateAggregates();
    const keys = ['b2b', 'b2cl', 'b2cs', 'cdnr', 'cdnur', 'exp', 'at', 'atadj', 'exemp', 'hsn_b2b', 'hsn_b2c', 'eco'];
    const labels = {
        b2b: 'B2B', b2cl: 'B2CL', b2cs: 'B2CS', cdnr: 'CDNR', cdnur: 'CDNUR', exp: 'EXP', at: 'AT', atadj: 'ATADJ', exemp: 'EXEMP',
        hsn_b2b: 'HSN (B2B)', hsn_b2c: 'HSN (B2C)', eco: 'ECO'
    };
    ...
    let aoa = [
        ...
        ['GSTIN:', gstinInput, 'Return Period:', fpInput],
        ['Generated On:', new Date().toLocaleString(), '', ''],
        [],
        ['Section', 'Record Count', 'Taxable Value', 'IGST', 'CGST', 'SGST', 'CESS']
    ];

    let totCnt = 0, totTx = 0, totI = 0, totC = 0, totS = 0, totCess = 0;
    ...
    keys.forEach(k => {
        ...
        if (data && data.count > 0) {
            ...
            if (!k.startsWith('hsn')) {
                ...
                totC += data.camt || 0;
                totS += data.samt || 0;
                totCess += data.csamt || 0;
            }
        }
    });

    aoa.push(['GRAND TOTAL (Excl. HSN)', totCnt, totTx, totI, totC, totS, totCess]);
    
    aoa.push([]);
    aoa.push(['Other Details']);
    aoa.push(['DOCS (Total Net Issued)', agg.docs.count]);
    aoa.push(['MASTER (Records Loaded)', agg.master.count]);

    const ws = XLSX.utils.aoa_to_sheet(aoa);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Summary");
    XLSX.writeFile(wb, `GSTR1_Summary_${fpInput}_${gstinInput}.xlsx`);
}
...
function downloadPDFSummary() {
    ...
    let html = `
        <div style="text-align: center; border-bottom: 2px solid #000; padding-bottom: 10px; margin-bottom: 20px;">
            <h1 style="margin: 0; font-size: 24px;">GSTR-1 Summary Report</h1>
            <p style="margin: 5px 0 0 0; color: #555;">Generated by CM Experts Group</p>
        </div>
        <table style="width: 100%; margin-bottom: 20px; font-size: 14px;">
            <tr>
                <td style="padding: 5px 0;"><strong>Client Name:</strong> ${clientName}</td>
                <td style="padding: 5px 0;"><strong>Client ID:</strong> ${clientId}</td>
            </tr>
            <tr>
                <td style="padding: 5px 0;"><strong>GSTIN:</strong> ${gstinInput}</td>
                <td style="padding: 5px 0;"><strong>Return Period:</strong> ${fpInput}</td>
            </tr>
            <tr>
                <td colspan="2" style="padding: 5px 0;"><strong>Generated On:</strong> ${new Date().toLocaleString('en-IN')}</td>
            </tr>
        </table>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            <thead>
                <tr style="background-color: #f0f0f0;">
                    <th style="border: 1px solid #ccc; padding: 10px; text-align: left;">Section</th>
                    <th style="border: 1px solid #ccc; padding: 10px; text-align: right;">Count</th>
                    <th style="border: 1px solid #ccc; padding: 10px; text-align: right;">Taxable Value</th>
                    <th style="border: 1px solid #ccc; padding: 10px; text-align: right;">IGST</th>
                    <th style="border: 1px solid #ccc; padding: 10px; text-align: right;">CGST</th>
                    <th style="border: 1px solid #ccc; padding: 10px; text-align: right;">SGST</th>
                    <th style="border: 1px solid #ccc; padding: 10px; text-align: right;">CESS</th>
                </tr>
            </thead>
            <tbody>
    `;
    ...
    keys.forEach(k => {
        ...
        if (data && data.count > 0) {
            html += `
                <tr>
                    <td style="border: 1px solid #ccc; padding: 8px;">${labels[k]}</td>
                    <td style="border: 1px solid #ccc; padding: 8px; text-align: right;">${data.count}</td>
                    <td style="border: 1px solid #ccc; padding: 8px; text-align: right;">₹${(data.txval || 0).toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td style="border: 1px solid #ccc; padding: 8px; text-align: right;">₹${(data.iamt || 0).toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td style="border: 1px solid #ccc; padding: 8px; text-align: right;">₹${(data.camt || 0).toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td style="border: 1px solid #ccc; padding: 8px; text-align: right;">₹${(data.samt || 0).toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td style="border: 1px solid #ccc; padding: 8px; text-align: right;">₹${(data.csamt || 0).toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                </tr>
            `;
            if (!k.startsWith('hsn')) {
                totCnt += data.count;
    ...
    html += `
            ...
            <tfoot>
                <tr style="background-color: #e8eaed; font-weight: bold;">
                    <td style="border: 1px solid #ccc; padding: 10px;">GRAND TOTAL (Excl. HSN)</td>
                    <td style="border: 1px solid #ccc; padding: 10px; text-align: right;">${totCnt}</td>
                    <td style="border: 1px solid #ccc; padding: 10px; text-align: right;">₹${totTx.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td style="border: 1px solid #ccc; padding: 10px; text-align: right;">₹${totI.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td style="border: 1px solid #ccc; padding: 10px; text-align: right;">₹${totC.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td style="border: 1px solid #ccc; padding: 10px; text-align: right;">₹${totS.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                    <td style="border: 1px solid #ccc; padding: 10px; text-align: right;">₹${totCess.toLocaleString('en-IN', {minimumFractionDigits: 2})}</td>
                </tr>
            </tfoot>
        </table>
        
        <div style="margin-top: 20px; font-size: 13px;">
            <p><strong>DOCS (Total Net Issued):</strong> ${agg.docs.count}</p>
            <p><strong>MASTER (Records Loaded):</strong> ${agg.master.count}</p>
        </div>
    `;
    ...
    const opt = {
        margin:       0.5,
        filename:     `GSTR1_Summary_${fpInput}_${gstinInput}.pdf`,
